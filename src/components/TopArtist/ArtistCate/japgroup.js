...
class JapGroup extends React.Component {
	constructor(props) {
		super(props);
	...
	async componentDidMount() {
		const japGroup = await fetchget("/api/artist/list?cat=6003")
		this.setState({ japGroups: japGroup.artists })
	}

	render() {	
		const hotArtistBoxDetail = this.state.japGroups;
		return(<div className='artistCateList'>	
			<div className='artistCateCon'>
				...
				<div className='artistCateNav'>			
					...
					<div className='suggest'>
						...
						<ul>
							...
							<li><Link to='/japgroup' style={{color:"#c62f2f"}}>日本组合/乐队</Link></li>
						</ul>
						</Router>
					</div>
				</div>
				...
				<div className='artistCateConBox'>
						...
						<div className='cateBoxTitle'>日本组合/乐队</div>
